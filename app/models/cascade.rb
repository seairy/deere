class Cascade < ApplicationRecord
  ...
  enumerize :action_when_owner_destroyed, in: %w(nothing destroy delete_all nullify restrict_with_exception restrict_with_error), predicates: { prefix: true }, scope: true
  enumerize :action_when_child_destroyed, in: %w(nothing destroy delete), predicates: { prefix: true }, scope: true
  validates :source_alias, length: { maximum: 100 }, format: /\A\w+\Z/, allow_blank: true
  validates :destination_alias, length: { maximum: 100 }, format: /\A\w+\Z/, allow_blank: true
